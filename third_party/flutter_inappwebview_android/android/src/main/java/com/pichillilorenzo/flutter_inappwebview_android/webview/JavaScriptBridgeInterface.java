...
public class JavaScriptBridgeInterface {
  ...
  public void _callHandler(final String handlerName, final String _callHandlerID, final String args) {
    // ZRemote security patch: this interface is callable by every frame. Reject
    // unsafe callback IDs before posting work or interpolating an ID into JS.
    if (!JavaScriptBridgeInputValidator.isValid(handlerName, _callHandlerID, args)) {
      return;
    }
    if (inAppWebView == null) {
      return;
